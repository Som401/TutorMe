...
const getRatings = async (request, response) => {
  try {
    ...
    response.status(500).json({ msg: 'Error fetching ratings', error: error.message });
  }
};
  const postRating = async (request, response) => {
    try {
      const {rating, tutorEmail, studentName} = request.body;
  ...
  const checkRatingExists = async (request, response) => {
    try {
      const { tutorEmail, studentName } = request.params;
  
      // Find the student and tutor based on email and name
      const student = await Student.findOne({
        where: {
          Name: studentName,
        },
      ...
      response.status(500).json({ msg: 'Error checking rating existence', error: error.message });
    }
  };
